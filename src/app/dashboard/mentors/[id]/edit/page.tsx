...
export default function AdminEditMentorProfilePage() {
  const { id } = useParams() as { id: string };
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [targetAdmin, setTargetAdmin] = useState<{ name: string | null; email: string } | null>(null);
  ...
  useEffect(() => {
    fetch(`/api/org/admins/${id}/profile`)
      .then(async (res) => {
        if (res.status === 401 || res.status === 403) {
          toast.error("You don't have permission to edit mentor profiles");
          return;
        }
        if (!res.ok) {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch(`/api/org/admins/${id}/profile`, {
        method: "POST",
  ...
  if (!targetAdmin) {
    return (
      <div className="flex items-center justify-center py-24">
        <p className="text-slate-400">Team member not found or access denied.</p>
      </div>
    );
  }

  const displayName = targetAdmin.name ?? targetAdmin.email;
      ...
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2">
            ...
            Editing Mentor Profile
          </h1>
          <p className="text-sm text-slate-400 mt-1">
            Editing public profile details for {displayName}.
          </p>
        </div>
        ...
        <div className="glass-card p-6 space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Tell potential mentees about this mentor's background..."
            />
          </div>
