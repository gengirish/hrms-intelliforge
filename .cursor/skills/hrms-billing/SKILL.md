...
# Interview Bot Stripe Billing

## Subscription Plans

| Plan | Price | Interviews/mo | Formats | Users | Features |
|------|-------|---------------|---------|-------|----------|
| Free Trial | $0 (14 days) | 10 | Text only | 1 | Basic reports |
| Starter | $99/mo | 50 | Text only | 2 | Reports, email notifications |
| Professional | $299/mo | 200 | Text + Voice | 10 | API access, analytics, ATS webhooks |
| Enterprise | $799/mo | Unlimited | All formats | Unlimited | Custom branding, SSO, priority support |

## Payment Flow

```
1. Org signs up → Free trial (14 days, 10 interviews)
...
```python
# src/interviewbot/services/billing_service.py
import stripe
...
PLAN_LIMITS = {
    "free": {"interviews": 10, "users": 1, "formats": ["text"]},
    "starter": {"interviews": 50, "users": 2, "formats": ["text"]},
    "professional": {"interviews": 200, "users": 10, "formats": ["text", "voice"]},
    "enterprise": {"interviews": -1, "users": -1, "formats": ["text", "voice", "video"]},
}
...
class BillingService:
    ...
    async def check_interview_quota(self, org_id: str) -> dict:
        sub = await self._get_subscription(org_id)
        limits = PLAN_LIMITS.get(sub.plan_tier, PLAN_LIMITS["free"])
        remaining = limits["interviews"] - sub.interviews_used if limits["interviews"] > 0 else -1
    ...
    async def increment_usage(self, org_id: str):
        await self.db.execute(
            "UPDATE subscription SET interviews_used = interviews_used + 1 WHERE org_id = :org_id",
            {"org_id": org_id},
        )
...
```python
# src/interviewbot/routers/billing.py
from fastapi import APIRouter, Request, HTTPException, status, Depends, Header
import stripe
from interviewbot.config import get_settings
from interviewbot.dependencies import get_db, get_current_user, get_org_id
...
async def get_subscription(
    ...
    service = BillingService(db)
    return await service.check_interview_quota(str(org_id))
```

## Plan Enforcement Middleware

Check quota before starting any interview.

```python
# In interview service, before creating a session:
async def enforce_quota(db, org_id: str, requested_format: str):
    service = BillingService(db)
    quota = await service.check_interview_quota(org_id)

    if not quota["can_interview"]:
        raise HTTPException(
            status.HTTP_402_PAYMENT_REQUIRED,
            f"Interview quota exceeded. Used {quota['used']}/{quota['limit']}. Please upgrade.",
        )

    if requested_format not in quota["allowed_formats"]:
        raise HTTPException(
            status.HTTP_403_FORBIDDEN,
            f"Format '{requested_format}' not available on {quota['plan']} plan. Upgrade to access.",
        )
```
...
4. **Check quota before every interview** -- enforce at service layer
5. **Reset usage counter monthly** -- on `invoice.paid` webhook
6. **Stripe Customer Portal** for self-service plan management -- don't build custom billing UI
7. **Free trial = 14 days, 10 interviews** -- no credit card required
8. **Plan tiers control format access** -- starter=text, pro=text+voice, enterprise=all
